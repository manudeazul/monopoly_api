...
- `jogadores` — todos os jogadores ordenados por saldo final (maior para menor)

---

### `GET /tabuleiro`

Retorna o estado atual do tabuleiro carregado em memória.
...
{
  "propriedades": [
    { "posicao": 0, "custo": 120, "aluguel": 15 },
    { "posicao": 1, "custo": 200, "aluguel": 40 },
    ...
  ]
}
```
...
Substitui o tabuleiro atual e persiste as alterações no arquivo `board.csv`.
...
Regras de validação:
- Deve conter exatamente **20 propriedades**
...
| Classe | O que testa |
|---|---|
| `TestSimulacao` | Comportamentos de cada jogador, carregamento do CSV, volta no tabuleiro, vencedor |
| `TestSimularEndpoint` | Status HTTP, estrutura do JSON, validade do vencedor em múltiplas chamadas |
